...
function updateGUI()
{
    ...
    if(perSec > 0)
    {
        document.getElementById('perSecond').textContent = "You gain $" + format(perSec) + " per second."
    }
    for (let i = 0; i < 10; i++)
    {
        let g = generators[i]
        ...
        if(g.cost > money) document.getElementById("gen" + (i + 1)).classList.add("unavailable")
        else document.getElementById("gen" + (i + 1)).classList.remove("unavailable")
    }
}

function productionLoop(diff)
{
    let add = generators[0].amount * generators[0].mult * diff
    money += add
    perSec = (add / diff)
    for (let i = 1; i < 10; i++)
    {
